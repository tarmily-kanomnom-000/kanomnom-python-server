...
@router.post("/{instance_index}/products/description-metadata", response_model=GrocyProductsResponse)
async def update_product_description_metadata(
    ...
    def _apply_updates():
        return with_grocy_manager(
            instance_index,
            lambda manager: manager.update_product_description_metadata(_build_product_metadata_updates(payload)),
        )
...
def _build_product_metadata_updates(
    ...
    for update in payload.updates:
        ...
        if not conversions and not (update.description or "").strip():
            raise ValueError("description or unit conversions must include at least one entry.")
