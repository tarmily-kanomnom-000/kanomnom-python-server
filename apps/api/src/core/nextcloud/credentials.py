...
class NextcloudCredentialsRepository:
    """Loads Nextcloud credentials from instance manifests."""

    def __init__(self, manifest_root: Path) -> None:
        self.manifest_root = manifest_root

    def load(self, instance_key: str) -> NextcloudInstanceCredentials:
...
def _resolve_credentials(
    ...
    username = _require_string(resolved.get("username"), "credentials.username")
    password = _require_string(resolved.get("password"), "credentials.password")
    return NextcloudInstanceCredentials(username=username, password=password)
