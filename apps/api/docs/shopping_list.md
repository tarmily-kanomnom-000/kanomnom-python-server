...
- **Responses:** `201` with item JSON; `404` if no active list; `404` if product_id not found; `409` if product_id already on the list.
- `POST /grocy/{instance_index}/shopping-list/active/items/bulk` — bulk add items (Design-for-N).
- **Request:** array of `{ "product_id": <int>, "quantity": <float> }` (non-empty; duplicates rejected).
- **Notes:** Single lock/read/write per batch; shared enrichment (location name, stock totals, min stock, unit, last purchase price); rejects duplicate `product_id` in the payload or already on the list. Version/last_modified update once per batch.
- **Responses:** `201` with array of items; `404` if no active list; `404` if any product_id not found; `400` for empty/duplicate payload; `409` if product already on the list.
- `PATCH /grocy/{instance_index}/shopping-list/items/bulk` — bulk update items (status/notes/quantity/location).
- **Request:** `{ "updates": [ { "item_id": "uuid", "status": "pending|purchased|unavailable", "quantity_purchased": <float|null>, "notes": <string|null>, "checked_at": <iso|null>, "shopping_location_id": <int|null>, "shopping_location_name": <string|null> } ] }` (non-empty; unique `item_id`s).
- **Notes:** `shopping_location_id` / `shopping_location_name` may be supplied together to reassign an item. Passing `shopping_location_id: null` will set the name to `UNKNOWN`. Location order is refreshed from the updated item set for N>1 updates.
- **Responses:** `200` with updated items; `404` if list missing or any item_id missing; `400` invalid payload or duplicate item_id.
...
- **Notes:** Refreshes `location_order` after removal to drop empty locations while preserving ordering.
