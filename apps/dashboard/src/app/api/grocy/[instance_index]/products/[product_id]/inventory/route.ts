...
export async function POST(
  ...
  const newAmountRaw =
    (payload as Record<string, unknown>).newAmount ??
    (payload as Record<string, unknown>).new_amount;
  const newAmount = Number(newAmountRaw);
  if (!Number.isFinite(newAmount)) {
    return NextResponse.json(
      { error: "newAmount must be a valid number" },
      { status: 400 },
    );
  }
  ...
  const url = new URL(
    `/grocy/${instance_index}/products/${product_id}/inventory`,
    apiBaseUrl,
  );
    ...
    return NextResponse.json(
      {
        error:
          detail ||
          `Failed to submit inventory correction (${upstreamResponse.status}).`,
      },
