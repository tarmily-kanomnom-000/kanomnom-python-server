...
class SemanticIngredientMatcher:
    ...
    def _filter_materials_by_fuzzy_score(self, unmatched_ingredients: list[str], available_materials: list[str]) -> list[str]:
        ...
        logger.info(f"📊 Fuzzy filtering: {original_count} materials → {filtered_count} candidates ({filtered_count/original_count*100:.1f}%)")
...
Now match the ingredients:"""
    ...
    def find_ingredient_matches(self, unmatched_ingredients: list[str], available_materials: list[str]) -> dict[str, str]:
        ...
        try:
            ...
            if response:
                ...
                logger.info(f"✅ LLM call completed: {len(successful_matches)} matches found out of {len(unmatched_ingredients)} ingredients")
    ...
    def _parse_matching_response(self, response: str, unmatched_ingredients: list[str], available_materials: list[str]) -> dict[str, str]:
...
def get_semantic_matcher(api_client: Callable = None, fuzzy_threshold: float = 40.0, max_candidates: int = 2) -> SemanticIngredientMatcher:
